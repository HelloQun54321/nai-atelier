  ...
  return <>
    {view === 'home' && (
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <div className="mt-1.5 truncate text-base font-black text-gray-900 dark:text-white">{config?.configured ? currentModel?.name || config.model : '尚未配置模型服务'}</div>
        ...
        {config?.visionAvailable && <div className="mt-2.5 border-t border-gray-100 pt-2 text-xs text-gray-500 dark:border-gray-800">视觉模型：<b className="text-gray-700 dark:text-gray-200">{currentVisionModel?.name || config.visionModel}</b> · {currentVisionModel?.provider}{config.visionDedicated ? ' · 独立分析' : ' · 跟随主模型'} · {config.visionMode === 'auto' ? '自动匹配' : '手动固定'}</div>}
      </div>
