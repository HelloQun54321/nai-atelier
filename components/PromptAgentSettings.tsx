...
type View = 'home' | 'login' | 'logout' | 'model' | 'key';
...
export const PromptAgentSettings: React.FC<PromptAgentSettingsProps> = ({ notify }) => {
  ...
  const [loginEvents, setLoginEvents] = useState<Array<{ type: string; message?: string; links?: Array<{ url: string; label?: string }> }>>([]);
  ...
  const closeView = () => { setView('home'); setQuery(''); setTargetProvider(null); setApiKey(''); setLoginAnswers([]); setLoginPrompt(null); setLoginEvents([]); setShowKey(false); };
  ...
  const advanceLogin = async (provider: PromptAgentProvider, answers: string[]) => {
    setBusy(true);
    try {
      const result = await promptAgentService.login(provider.id, answers);
      if (result.complete) {
        await reload();
