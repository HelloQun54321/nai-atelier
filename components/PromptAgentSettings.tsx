    ...
    <div className="mx-auto max-w-2xl space-y-4 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 md:p-6">
      ...
      <div>
        ...
        <div className="space-y-3">{value.models.map((model, index) => <div key={index} className="rounded-2xl border border-gray-200 p-3 dark:border-gray-700">
          ...
          <div className="mt-2 flex flex-wrap gap-4"><label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300"><input type="checkbox" checked={model.imageInput} onChange={event => patchModel(index, { imageInput: event.target.checked })}/>支持识图</label><label className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300"><input type="checkbox" checked={model.reasoning} onChange={event => patchModel(index, { reasoning: event.target.checked })}/>支持推理</label></div>
        </div>)}</div>
      </div>
      <div className="flex gap-2 border-t border-gray-100 pt-4 dark:border-gray-800"><button type="button" disabled={busy || !ready} onClick={onTest} className="mobile-touch flex-1 rounded-xl border border-indigo-300 text-sm font-bold text-indigo-600 disabled:opacity-40 dark:border-indigo-800 dark:text-indigo-300">测试连接</button><button type="button" disabled={busy || !ready} onClick={onSave} className="mobile-touch flex-[1.4] rounded-xl bg-gradient-to-r from-fuchsia-600 to-indigo-600 text-sm font-bold text-white disabled:opacity-40">保存并选用</button></div>
      <p className="text-[10px] leading-4 text-gray-400">API Key只会加密保存在运行项目的电脑。测试连接会由电脑请求该地址的 /models，不经过手机。</p>
    </div>
  </div>;
};
  ...
  return <>
    <div className="mt-3 space-y-3">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <button type="button" onClick={() => openCustom()} className="mobile-touch rounded-xl border border-fuchsia-200 bg-fuchsia-50 px-3 text-left text-sm font-bold text-fuchsia-700 shadow-sm dark:border-fuchsia-900 dark:bg-fuchsia-950/30 dark:text-fuchsia-300"><span className="mr-2">⌁</span>添加自定义接口</button>
      </div>
      {customProviders.length > 0 && <div className="rounded-2xl border border-gray-200 bg-white p-2 dark:border-gray-800 dark:bg-gray-900"><div className="px-2 py-1 text-[11px] font-bold text-gray-400">自定义接口</div>{customProviders.map(provider => <div key={provider.id} className="flex items-center gap-2 rounded-xl px-2 py-2 hover:bg-gray-50 dark:hover:bg-gray-800"><span className="h-2.5 w-2.5 rounded-full bg-fuchsia-500"/><div className="min-w-0 flex-1"><b className="block truncate text-sm dark:text-white">{provider.name}</b><span className="block truncate text-[10px] text-gray-400">{provider.baseUrl} · {provider.models.length} 个模型</span></div><button type="button" onClick={() => openCustom(provider)} className="mobile-touch rounded-xl px-3 text-xs font-bold text-indigo-600">编辑</button><button type="button" onClick={() => void deleteCustom(provider)} className="mobile-touch rounded-xl px-3 text-xs font-bold text-red-500">删除</button></div>)}</div>}
