...
export const PromptAgentSettings: React.FC<PromptAgentSettingsProps> = ({ notify }) => {
  ...
  const fetchCustom = async () => {
    ...
    try {
      ...
      const existing = new Set(customDraft.models.map(model => model.id.trim().toLowerCase()).filter(Boolean));
      const added = result.models.filter(id => !existing.has(id.toLowerCase()));
      if (!added.length) { notify('接口返回的模型已全部在列表中', 'success'); return; }
      setCustomDraft(previous => ({ ...previous, models: [...previous.models, ...added.map(id => ({ id, name: '', reasoning: false, imageInput: false, contextWindow: 128000, maxTokens: 16384 }))] }));
      notify(`已获取 ${added.length} 个模型，请按需勾选识图/推理并保存${added.length < result.models.length ? `（其中 ${result.models.length - added.length} 个已存在）` : ''}`);
