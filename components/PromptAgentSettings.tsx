...
export const PromptAgentSettings: React.FC<PromptAgentSettingsProps> = ({ notify }) => {
  ...
  const closeView = () => { setView('home'); setQuery(''); setTargetProvider(null); setApiKey(''); setLoginAnswers([]); setLoginAuthType('api_key'); setLoginPrompt(null); setLoginEvents([]); setShowKey(false); };
  ...
  const advanceLogin = async (provider: PromptAgentProvider, answers: string[], authType = loginAuthType) => {
    setBusy(true);
    try {
      setLoginAuthType(authType);
      const result = await promptAgentService.login(provider.id, answers, authType);
      if (result.complete) {
        await reload();
