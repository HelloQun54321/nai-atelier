...
const AgentMessageList = React.memo(({
  ...
  return <>
    ...
    {visibleMessages.map((message, index) => <div key={message.id} className={`group min-w-0 max-w-[92%] [overflow-wrap:anywhere] rounded-2xl px-3 pb-2 pt-2.5 text-sm leading-6 ${message.role === 'user' ? 'ml-auto whitespace-pre-wrap bg-indigo-600 text-white' : message.role === 'error' ? 'whitespace-pre-wrap bg-red-50 text-red-600 dark:bg-red-950/40 dark:text-red-300' : 'bg-white text-gray-800 shadow-sm dark:bg-gray-900 dark:text-gray-100'}`}>
...
export const PromptAgentPanel: React.FC<PromptAgentPanelProps> = props => {
  ...
  const updateCreativeMode = async (creativeMode: boolean) => {
    ...
    try {
      ...
      setMessages(previous => [...previous, { id: crypto.randomUUID(), role: 'error', text: error instanceof Error ? error.message : '切换创作模式失败' }]);
    }
  };
    ...
    <section className="relative flex min-w-0 flex-1 flex-col">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col overflow-hidden">
        ...
        <div className="border-t border-gray-200 bg-white p-2 pb-[max(.5rem,env(safe-area-inset-bottom))] dark:border-gray-800 dark:bg-gray-900 md:p-3">
          ...
          <div className="flex min-w-0 items-end gap-2"><label title={activeModel?.imageInput ? '添加图片' : '当前模型不支持识图'} className={`mobile-touch flex h-10 w-10 flex-none items-center justify-center rounded-xl border border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400 ${activeModel?.imageInput ? 'cursor-pointer hover:border-indigo-400 hover:text-indigo-500' : 'cursor-not-allowed opacity-35'}`}><ImagePlus className="h-[18px] w-[18px]" /><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple hidden onChange={event => { addAttachments(event.target.files); event.currentTarget.value = ''; }} disabled={running || attachments.length >= 4 || !activeModel?.imageInput} /></label><textarea value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void run(); } }} rows={1} placeholder={running ? (queueMode === 'steer' ? '补充或纠正当前任务…' : '添加完成后继续处理的任务…') : '告诉 Agent 你想让它查看、修改或生成什么…'} className="h-10 min-h-10 min-w-0 flex-1 resize-none rounded-xl border border-gray-300 bg-gray-50 px-3 py-2 text-sm leading-5 text-gray-900 outline-none placeholder:text-gray-400 focus:border-indigo-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder:text-gray-600" /><button type="button" onClick={() => void run()} disabled={!input.trim() && !attachments.length} className="mobile-touch flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-indigo-600 p-0 text-white shadow-lg shadow-indigo-500/15 transition-colors hover:bg-indigo-500 disabled:bg-gray-200 disabled:text-gray-400 disabled:shadow-none dark:disabled:bg-gray-800 dark:disabled:text-gray-600" aria-label={running ? '追加要求' : editingMessageId ? '重新发送' : '执行'} title={running ? '追加要求' : editingMessageId ? '重新发送' : '执行'}><Send className="h-[18px] w-[18px]" /></button></div>
        </div>
      </main>
    </section>
    </div>
