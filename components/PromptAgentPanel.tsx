      ...
      <div className="flex-1 space-y-1 overflow-y-auto px-2 pb-3">
        {sessions.filter(session => session.title.toLowerCase().includes(sessionSearch.trim().toLowerCase())).map(session => <div key={session.id} className={`group relative min-h-[4.75rem] rounded-xl border ${session.id === activeSessionId ? 'border-indigo-300 bg-indigo-50 dark:border-indigo-800 dark:bg-indigo-950/30' : 'border-transparent hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
          {editingSessionId === session.id ? <form onSubmit={event => { event.preventDefault(); void saveSessionTitle(session); }} className="flex min-h-[4.75rem] items-center px-2 pr-12"><input autoFocus value={editingTitle} onChange={event => setEditingTitle(event.target.value)} onBlur={() => void saveSessionTitle(session)} className="h-9 min-w-0 flex-1 rounded-lg border border-indigo-300 bg-white px-2 text-xs text-gray-800 outline-none dark:bg-gray-950 dark:text-gray-100" /></form> : <button type="button" disabled={running && session.id !== activeSessionId} onClick={() => { if (!running) { setActiveSessionId(session.id); setShowSessions(false); setSessionMenuId(''); } }} className="block min-h-[4.75rem] w-full py-2 pl-3 pr-12 text-left">
            ...
            <span className="mt-0.5 block truncate text-[10px] text-gray-400">{session.model} · {session.messageCount || 0} 轮 · {new Date(session.updatedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
            <span className="mt-1 flex items-center gap-1.5 text-[9px] font-bold"><span className={`rounded-full px-1.5 py-0.5 ${session.creativeMode ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-950/60 dark:text-indigo-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'}`}>破限{session.creativeMode ? '开' : '关'}</span>{session.running ? <span className="text-indigo-600">工作中</span> : session.taskStatus === 'interrupted' ? <span className="text-amber-600">上次中断</span> : session.taskStatus === 'failed' ? <span className="text-red-500">上次失败</span> : <span className="text-emerald-600">就绪</span>}</span>
          </button>}
    ...
    <section className="relative flex min-w-0 flex-1 flex-col">
      <header className="border-b border-gray-200 bg-white pt-[env(safe-area-inset-top)] dark:border-gray-800 dark:bg-gray-900">
        <div className="flex h-12 items-center gap-1 px-2 md:px-3">
          ...
          <div className="min-w-0 flex-1"><h2 className="truncate text-sm font-black text-gray-900 dark:text-white">{activeSession?.title || '项目 Agent'}</h2><p className="truncate text-[10px] text-gray-500">{activeSession?.model || '未选择模型'} · {activeModel?.imageInput ? '支持识图' : '不支持识图'}</p></div>
          <div ref={modelMenuRef} className="relative flex items-center gap-1">
            ...
            {showModelMenu && <div className="absolute right-0 top-11 z-30 flex max-h-[min(76vh,42rem)] w-[min(20rem,calc(100vw-1rem))] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
              <div className="border-b border-gray-100 p-3 dark:border-gray-800"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><b className="block text-xs text-gray-700 dark:text-gray-100">模型与思考</b><span className="mt-0.5 block truncate text-[10px] text-gray-400">{activeSession?.model || '未选择'}</span></div><span className="shrink-0 rounded-full bg-indigo-50 px-2 py-1 text-[10px] font-bold text-indigo-600 dark:bg-indigo-950/50 dark:text-indigo-300">最高：{thinkingLevelLabels[availableThinkingLevels.at(-1) || 'off']}</span></div><label className="mt-3 flex items-center gap-2 text-[11px] text-gray-500"><span className="flex-1">思考等级</span><select aria-label="思考等级" disabled={running || availableThinkingLevels.length <= 1} value={selectedThinkingLevel} onChange={event => void updateThinkingLevel(event.target.value as PromptAgentThinkingLevel)} className="h-9 min-w-24 rounded-lg border border-gray-200 bg-gray-50 px-2 text-xs font-bold text-gray-700 outline-none disabled:opacity-40 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-200">{availableThinkingLevels.map(level => <option key={level} value={level}>{thinkingLevelLabels[level]}</option>)}</select></label>{activeSession && !activeSession.creativeModeLocked && !activeSession.messageCount && <div className="mt-3 flex items-center justify-between gap-2 border-t border-gray-100 pt-2.5 text-[11px] text-gray-500 dark:border-gray-800"><span>破限模式（首条消息前可选）</span><button type="button" role="switch" aria-checked={activeSession.creativeMode} aria-label="切换本对话破限模式" onClick={() => void updateCreativeMode(!activeSession.creativeMode)} className={`relative h-5 w-10 shrink-0 rounded-full transition-colors ${activeSession.creativeMode ? 'bg-indigo-500' : 'bg-gray-300 dark:bg-gray-700'}`}><span className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${activeSession.creativeMode ? 'translate-x-5' : 'translate-x-0.5'}`} /></button></div>}{activeSession?.creativeModeLocked && <p className="mt-3 border-t border-gray-100 pt-2.5 text-[10px] text-gray-400 dark:border-gray-800">破限模式已随首条消息锁定。</p>}</div>
              <div className="overflow-y-auto p-2"><button type="button" onClick={() => { setShowModelMenu(false); props.onClose(); window.dispatchEvent(new CustomEvent('nai-open-global-settings', { detail: { section: 'agent' } })); }} className="mb-2 flex w-full items-center justify-between rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-left text-xs font-bold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-900 dark:bg-indigo-950/30 dark:text-indigo-300 dark:hover:bg-indigo-950/50"><span>管理模型服务</span><span aria-hidden="true">→</span></button>{models.map(model => <button key={`${model.provider}/${model.id}`} type="button" onClick={() => void updateSessionModel(model)} className={`block w-full rounded-xl px-3 py-2 text-left ${model.provider === activeSession?.provider && model.id === activeSession?.model ? 'bg-indigo-50 dark:bg-indigo-950/40' : 'hover:bg-gray-50 dark:hover:bg-gray-800'}`}><b className="block truncate text-xs text-gray-800 dark:text-white">{model.id}</b><span className="block text-[10px] text-gray-400">{model.provider} · {model.reasoning ? `推理，最高 ${thinkingLevelLabels[model.thinkingLevels.at(-1) || 'off']}` : '普通'}{model.imageInput ? ' · 识图' : ''}</span></button>)}</div>
              {!fullscreen && <div className="hidden border-t border-gray-100 p-3 md:block dark:border-gray-800"><div className="mb-2 text-[10px] font-bold text-gray-400">面板宽度</div><div className="grid grid-cols-3 gap-1">{[{ label: '窄', width: 440 }, { label: '标准', width: 540 }, { label: '宽', width: 680 }].map(item => <button key={item.width} type="button" onClick={() => choosePanelWidth(item.width)} className={`h-8 rounded-lg text-[11px] font-bold ${Math.abs(panelWidth - item.width) < 30 ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700'}`}>{item.label}</button>)}</div></div>}
            </div>}
          </div>
          <button type="button" onClick={() => void exportAuditLog()} disabled={exportingLog || !activeSessionId} className="mobile-touch flex h-9 items-center justify-center rounded-xl px-2 text-gray-500 hover:bg-gray-100 disabled:opacity-40 dark:text-gray-400 dark:hover:bg-gray-800" aria-label="导出本会话 Agent 日志" title="导出本会话 Agent 日志（含步骤、工具调用与流式响应）"><Download className="h-[17px] w-[17px]" /><span className="ml-1 hidden text-[11px] font-bold lg:inline">{exportingLog ? '导出中' : '日志'}</span></button>
        ...
        <div className="flex min-h-9 items-center gap-2 border-t border-gray-100 px-3 py-1.5 text-[10px] dark:border-gray-800 md:px-4"><span className={`flex items-center gap-1.5 font-bold ${running ? 'text-indigo-600 dark:text-indigo-300' : editingMessageId ? 'text-amber-600 dark:text-amber-300' : 'text-emerald-600 dark:text-emerald-300'}`}><span className={`h-1.5 w-1.5 rounded-full ${running ? 'animate-pulse bg-indigo-500' : editingMessageId ? 'bg-amber-500' : 'bg-emerald-500'}`} />{executionStatus}</span><span className="hidden min-w-0 truncate text-gray-400 sm:block">{running ? '可在下方选择如何追加要求' : activeSession?.creativeMode ? '破限模式已选定' : '普通模式'}</span><span className="ml-auto flex items-center gap-1"><span className="hidden text-gray-400 sm:inline">{activeSession?.thinkingLevel ? `思考：${thinkingLevelLabels[activeSession.thinkingLevel]}` : ''}</span>{props.canUndo && <button type="button" onClick={props.onUndo} disabled={running} className="mobile-touch rounded-lg px-2 py-1 font-bold text-indigo-600 hover:bg-indigo-50 disabled:opacity-40 dark:text-indigo-300 dark:hover:bg-indigo-950/40">撤销修改</button>}<button type="button" onClick={() => void reset()} disabled={running || !messages.length} className="mobile-touch rounded-lg px-2 py-1 font-bold text-gray-500 hover:bg-gray-100 disabled:opacity-30 dark:hover:bg-gray-800">清空对话</button></span></div>
      </header>
      {logExportError && <div role="status" className="absolute right-3 top-[calc(5.25rem+env(safe-area-inset-top)+.25rem)] z-40 max-w-[min(28rem,calc(100%-1.5rem))] rounded-lg bg-red-50 px-2 py-1 text-[10px] font-bold text-red-600 shadow dark:bg-red-950/80 dark:text-red-300">{logExportError}</div>}
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col overflow-hidden">
        ...
        <div className="border-t border-gray-200 bg-white p-2 pb-[max(.5rem,env(safe-area-inset-bottom))] dark:border-gray-800 dark:bg-gray-900 md:p-3">
          ...
          <div className="flex min-w-0 items-end gap-2"><label title={activeModel?.imageInput ? '添加图片' : '当前模型不支持识图'} className={`mobile-touch flex h-11 w-11 flex-none items-center justify-center rounded-xl border border-gray-200 text-gray-500 dark:border-gray-700 dark:text-gray-400 ${activeModel?.imageInput ? 'cursor-pointer hover:border-indigo-400 hover:text-indigo-500' : 'cursor-not-allowed opacity-35'}`}><ImagePlus className="h-[18px] w-[18px]" /><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple hidden onChange={event => { addAttachments(event.target.files); event.currentTarget.value = ''; }} disabled={running || attachments.length >= 4 || !activeModel?.imageInput} /></label><textarea value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void run(); } }} rows={1} placeholder={running ? (queueMode === 'steer' ? '补充或纠正当前任务…' : '添加完成后继续处理的任务…') : editingMessageId ? '修改这条消息后重新发送…' : '告诉 Agent 你想让它查看、修改或生成什么…'} className="h-11 min-h-11 min-w-0 flex-1 resize-none rounded-xl border border-gray-300 bg-gray-50 px-3 py-2.5 text-sm leading-5 text-gray-900 outline-none placeholder:text-gray-400 focus:border-indigo-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100 dark:placeholder:text-gray-600" /><button type="button" onClick={() => void run()} disabled={!input.trim() && !attachments.length} className="mobile-touch flex h-11 w-11 flex-none items-center justify-center rounded-xl bg-indigo-600 p-0 text-white shadow-lg shadow-indigo-500/15 transition-colors hover:bg-indigo-500 disabled:bg-gray-200 disabled:text-gray-400 disabled:shadow-none dark:disabled:bg-gray-800 dark:disabled:text-gray-600" aria-label={running ? '追加要求' : editingMessageId ? '重新发送' : '执行'} title={running ? '追加要求' : editingMessageId ? '重新发送' : '执行'}><Send className="h-[18px] w-[18px]" /></button></div>
        </div>
      </main>
    </section>
    </div>
