...
export const InspirationGallery: React.FC<InspirationGalleryProps> = ({ currentUser, inspirationsData, onRefresh, notify, onNavigateToPlayground, chains = [], onCreateArtistChain, onSetChainCover }) => {
  ...
  const renderFilterControls = () => <div className="grid grid-cols-2 gap-3">
    <label className="text-sm font-bold text-gray-600 dark:text-gray-300 md:text-xs md:font-medium">分类<select value={collection} onChange={event => { setCollection(event.target.value as SmartCollection); setBoardId(''); }} className="mobile-touch mt-1.5 h-10 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm font-normal text-gray-800 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/10 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100"><option value="all">全部灵感</option><option value="unorganized">未整理</option><option value="pinned">已置顶</option><option value="recent">最近使用</option><option value="archived">已归档</option><option value="source:history">来源：生成历史</option><option value="source:aitag">来源：AITag</option><option value="source:upload">来源：手动上传</option><option value="source:agent">来源：项目 Agent</option><option value="source:other">来源：其他</option></select></label>
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-gray-50 dark:bg-gray-950">
    <WorkspaceToolbar className="relative z-30">
          ...
          {filtered.map(item => {
            ...
            return <MediaCardShell key={item.id} data-safe-mode-work="true" selected={selected} className="group relative flex flex-col">
              <div className="mobile-gallery-frame relative overflow-hidden md:aspect-square" style={{ '--mobile-image-ratio': `${item.params?.width || 832} / ${item.params?.height || 1216}` } as React.CSSProperties}>
                ...
                <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between p-2"><span className="inline-flex items-center gap-1 rounded-full border border-white/70 bg-white/90 px-2 py-1 text-[10px] font-bold text-gray-700 shadow-sm backdrop-blur dark:border-white/15 dark:bg-black/60 dark:text-white"><SourceIcon className="h-3 w-3" />{sourceLabel(item.sourceType)}</span>{item.isPinned && <span className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-white shadow"><Pin className="h-3.5 w-3.5 fill-current" /></span>}</div>
              ...
              <button type="button" onClick={() => setDetail(item)} className="min-w-0 flex-1 p-3 text-left"><div className="flex items-start gap-2"><h3 data-safe-mode-title="true" className="min-w-0 flex-1 truncate text-sm font-black text-gray-950 dark:text-white">{item.title}</h3>{(item.rating || 0) > 0 && <span className="inline-flex items-center gap-0.5 text-[11px] font-bold text-amber-500"><Star className="h-3 w-3 fill-current" />{item.rating}</span>}</div>{item.notes ? <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-gray-500 dark:text-gray-400">{item.notes}</p> : <p className="mt-1 truncate font-mono text-[10px] text-gray-400">{item.prompt || '尚未填写 Prompt'}</p>}{(item.tags || []).length > 0 && <div className="mt-2 flex gap-1 overflow-hidden">{item.tags?.slice(0, 3).map(tag => <span key={tag} className="max-w-24 truncate rounded-md bg-gray-100 px-1.5 py-0.5 text-[9px] font-semibold text-gray-500 dark:bg-gray-800 dark:text-gray-400">#{tag}</span>)}{(item.tags?.length || 0) > 3 && <span className="text-[9px] text-gray-400">+{(item.tags?.length || 0) - 3}</span>}</div>}<div className="mt-2 flex items-center justify-between text-[10px] text-gray-400"><span>{boardNameById.get(item.boardId || '') || '未整理'}</span><span>使用 {item.useCount || 0} 次</span></div></button>
            </MediaCardShell>;
          })}
    ...
    <MobileBottomSheet open={mobileFilters} title="筛选灵感" onClose={() => setMobileFilters(false)} footer={<div className="grid grid-cols-2 gap-3"><button type="button" onClick={resetFilters} className="mobile-touch rounded-xl border border-gray-300 text-sm font-bold text-gray-600 dark:border-gray-700 dark:text-gray-300">重置</button><button type="button" onClick={() => setMobileFilters(false)} className="mobile-touch rounded-xl bg-indigo-600 text-sm font-bold text-white shadow-lg shadow-indigo-600/20">查看 {filtered.length} 条结果</button></div>}><p className="mb-4 text-xs text-gray-400">分类、灵感板、标签与排序</p>{renderFilterControls()}</MobileBottomSheet>

    {boardEditor && <div className="ui-backdrop-enter fixed inset-0 z-[1060] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm md:items-center md:p-4" onClick={() => setBoardEditor(null)}><div className="ui-sheet-enter mobile-safe-bottom w-full max-w-sm rounded-t-3xl border border-gray-200 bg-white p-5 shadow-2xl dark:border-gray-700 dark:bg-gray-900 md:rounded-2xl" onClick={event => event.stopPropagation()}><h2 className="text-lg font-black">{boardEditor.id ? '编辑灵感板' : '新建灵感板'}</h2><input autoFocus value={boardEditor.name} onChange={event => setBoardEditor({ ...boardEditor, name: event.target.value })} placeholder="例如：电影感光影" className="mt-4 h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-950" /><div className="mt-4 flex gap-2">{BOARD_COLORS.map(color => <button key={color} type="button" onClick={() => setBoardEditor({ ...boardEditor, color })} className={`h-8 w-8 rounded-full transition ${boardEditor.color === color ? 'ring-2 ring-offset-2 dark:ring-offset-gray-900' : ''}`} style={{ backgroundColor: color }} aria-label={`颜色 ${color}`} />)}</div><div className="mt-6 grid grid-cols-2 gap-3 md:flex md:justify-end"><button type="button" onClick={() => setBoardEditor(null)} className="mobile-touch rounded-xl border border-gray-300 px-4 text-sm font-bold text-gray-600 dark:border-gray-700 dark:text-gray-300">取消</button><button type="button" disabled={!boardEditor.name.trim() || busy === 'board'} onClick={() => void saveBoard()} className="mobile-touch rounded-xl bg-indigo-600 px-4 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 disabled:opacity-40">保存</button></div></div></div>}

    {uploadOpen && <div className="ui-backdrop-enter fixed inset-0 z-[1050] flex items-center justify-center bg-black/60 p-0 backdrop-blur-sm md:p-5" onClick={() => setUploadOpen(false)}><div className="ui-modal-enter h-[100dvh] w-full max-w-3xl overflow-y-auto bg-white p-4 pt-[max(1rem,env(safe-area-inset-top))] shadow-2xl dark:bg-gray-900 md:h-auto md:max-h-[92vh] md:rounded-2xl md:border md:border-gray-700 md:p-6" onClick={event => event.stopPropagation()}><header className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-indigo-500">手动收录</p><h2 className="text-xl font-black">加入灵感库</h2></div><IconButton label="关闭" onClick={() => setUploadOpen(false)}><X /></IconButton></header><div className="mt-5 grid gap-5 md:grid-cols-[260px_1fr]"><button type="button" onClick={() => fileInputRef.current?.click()} className="flex aspect-[4/5] items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-gray-950">{uploadPreview ? <img src={uploadPreview} alt="上传预览" className="h-full w-full object-contain" /> : <span className="flex flex-col items-center gap-2 text-sm font-bold text-gray-400"><Upload className="h-7 w-7" />选择图片<span className="text-[10px] font-normal">PNG / JPEG / WebP，最多 12 MB</span></span>}</button><input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={event => void chooseFile(event.target.files?.[0])} /><div className="space-y-3"><label><span className="mb-1 block text-xs font-bold text-gray-500">标题</span><input data-safe-mode-title="true" value={uploadDraft.title} onChange={event => setUploadValue('title', event.target.value)} className="h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-950" /></label><div className="grid grid-cols-2 gap-3"><label><span className="mb-1 block text-xs font-bold text-gray-500">灵感板</span><select value={uploadDraft.boardId} onChange={event => setUploadValue('boardId', event.target.value)} className="h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-950"><option value="">未整理</option>{boards.map(board => <option key={board.id} value={board.id}>{board.name}</option>)}</select></label><label><span className="mb-1 block text-xs font-bold text-gray-500">标签</span><input value={uploadDraft.tags} onChange={event => setUploadValue('tags', event.target.value)} placeholder="构图, 光影" className="h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-950" /></label></div><label><span className="mb-1 block text-xs font-bold text-gray-500">备注</span><textarea value={uploadDraft.notes} onChange={event => setUploadValue('notes', event.target.value)} className="min-h-20 w-full rounded-xl border border-gray-300 bg-white p-3 text-sm dark:border-gray-700 dark:bg-gray-950" /></label><label><span className="mb-1 block text-xs font-bold text-gray-500">正面 Prompt</span><textarea value={uploadDraft.prompt} onChange={event => setUploadValue('prompt', event.target.value)} className="min-h-24 w-full rounded-xl border border-gray-300 bg-white p-3 font-mono text-xs dark:border-gray-700 dark:bg-gray-950" /></label><label><span className="mb-1 block text-xs font-bold text-gray-500">负面 Prompt</span><textarea value={uploadDraft.negativePrompt} onChange={event => setUploadValue('negativePrompt', event.target.value)} className="min-h-16 w-full rounded-xl border border-gray-300 bg-white p-3 font-mono text-xs dark:border-gray-700 dark:bg-gray-950" /></label></div></div><footer className="mt-6 grid grid-cols-2 gap-3 md:flex md:justify-end"><button type="button" onClick={() => setUploadOpen(false)} className="mobile-touch rounded-xl border border-gray-300 px-5 text-sm font-bold text-gray-600 dark:border-gray-700 dark:text-gray-300">取消</button><button type="button" disabled={!uploadFile || !uploadDraft.title.trim() || busy === 'upload'} onClick={() => void upload()} className="mobile-touch rounded-xl bg-indigo-600 px-5 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 disabled:opacity-40">{busy === 'upload' ? '正在保存…' : '加入灵感库'}</button></footer></div></div>}
