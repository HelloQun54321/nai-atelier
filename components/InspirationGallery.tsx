  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-gray-50 dark:bg-gray-950">
    <WorkspaceToolbar>
      ...
      <div className="relative hidden flex-none md:block">
        ...
        {desktopFilters && <div role="dialog" aria-label="筛选灵感" className="absolute left-1/2 top-[calc(100%+0.5rem)] z-50 w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 rounded-2xl border border-gray-200 bg-white p-4 shadow-2xl dark:border-gray-800 dark:bg-gray-900"><div className="mb-3 flex items-center justify-between"><div><h2 className="text-sm font-bold text-gray-900 dark:text-white">筛选灵感</h2><p className="mt-0.5 text-[11px] text-gray-400">按分类、灵感板、标签、评分和使用情况组合筛选</p></div>{activeFilterCount > 0 && <button type="button" onClick={resetFilters} className="text-xs font-bold text-indigo-600 dark:text-indigo-400">重置</button>}</div>{renderFilterControls()}</div>}
      </div>
