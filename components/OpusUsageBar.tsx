...
export const OpusUsageBar: React.FC<OpusUsageBarProps> = ({ collapsed }) => {
  const { usage, loading } = useNovelaiUsage();
  if (!usage) {
  ...
  return (
    <div
      ...
      className={`relative flex h-11 w-full cursor-default select-none items-center border-b border-gray-200 dark:border-gray-800 ${negative ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'} ${collapsed ? 'justify-center px-0' : 'justify-between px-3'}`}
    >
