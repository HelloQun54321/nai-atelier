...
export const InspirationDetail: React.FC<Props> = ({ item, items, boards, currentUser, chains, notify, onClose, onRefresh, onNavigateToPlayground, onCreateArtistChain, onSetChainCover, onOpenItem }) => {
  ...
  return <div className="ui-backdrop-enter fixed inset-0 z-[1050] flex items-center justify-center bg-black/80 p-0 backdrop-blur-sm md:p-6" onClick={closeLayer}>
    <div className="ui-modal-enter flex h-[100dvh] w-full max-w-7xl flex-col overflow-hidden bg-white shadow-2xl dark:bg-gray-950 md:h-[92vh] md:rounded-2xl md:border md:border-gray-700 lg:flex-row" onClick={event => event.stopPropagation()}>
      ...
      <section className="flex min-h-0 w-full flex-col border-l border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 lg:w-[560px]">
        ...
        <footer className="flex-none space-y-2 border-t border-gray-200 p-3 dark:border-gray-800">
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3"><button type="button" disabled={Boolean(busy)} onClick={() => void importToPlayground('replace')} className="mobile-touch col-span-2 flex items-center justify-center gap-2 rounded-xl bg-indigo-600 text-sm font-bold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-500 sm:col-span-1"><Play className="h-4 w-4" />完整导入</button><button type="button" disabled={Boolean(busy)} onClick={() => void importToPlayground('append-prompt')} className="mobile-touch rounded-xl border border-gray-300 bg-white text-xs font-bold text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">追加 Prompt</button><div className="relative"><button type="button" onClick={() => setMoreOpen(!moreOpen)} className="mobile-touch flex w-full items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white text-xs font-bold text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"><MoreHorizontal className="h-4 w-4" />更多复用<ChevronDown className="h-3.5 w-3.5" /></button>{moreOpen && <div className="absolute bottom-12 right-0 z-20 w-60 overflow-hidden rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl dark:border-gray-700 dark:bg-gray-900">{([['prompt-only', '仅使用正面 Prompt'], ['negative-only', '仅使用负面 Prompt'], ['params-only', '仅使用生成参数']] as [ImportMode, string][]).map(([mode, label]) => <button key={mode} type="button" onClick={() => void importToPlayground(mode)} className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800">{label}</button>)}<div className="my-1 border-t border-gray-200 dark:border-gray-800" /><button type="button" disabled={!onCreateArtistChain || Boolean(busy)} onClick={() => void createArtistChain()} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 disabled:opacity-40 dark:hover:bg-gray-800"><Palette className="h-4 w-4" />创建画师串</button><button type="button" disabled={Boolean(busy)} onClick={() => void createAsset('character')} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"><UserRound className="h-4 w-4" />创建角色参考</button><button type="button" disabled={Boolean(busy)} onClick={() => void createAsset('vibe')} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-800"><Sparkles className="h-4 w-4" />创建 Vibe</button></div>}</div></div>
          ...
          <div className="flex gap-2"><button type="button" onClick={() => { navigator.clipboard.writeText(draft.prompt); notify('Prompt 已复制'); }} className="mobile-touch flex flex-1 items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white text-xs font-bold text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"><Copy className="h-3.5 w-3.5" />复制</button><a href={draft.imageUrl} download={`${draft.title || 'inspiration'}.png`} className="mobile-touch flex flex-1 items-center justify-center gap-2 rounded-xl border border-gray-300 bg-white text-xs font-bold text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"><Download className="h-3.5 w-3.5" />下载</a>{editable && <button type="button" disabled={busy === 'save'} onClick={() => void save()} className="mobile-touch flex flex-1 items-center justify-center gap-2 rounded-xl bg-indigo-600 text-xs font-bold text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-500"><Check className="h-3.5 w-3.5" />保存整理</button>}</div>
        </footer>
      </section>
    </div>
  </div>;
