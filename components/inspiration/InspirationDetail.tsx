...
export const InspirationDetail: React.FC<Props> = ({ item, items, boards, currentUser, chains, notify, onClose, onRefresh, onNavigateToPlayground, onCreateArtistChain, onSetChainCover, onOpenItem }) => {
  ...
  return <div className="ui-backdrop-enter fixed inset-0 z-[1050] flex items-center justify-center bg-black/80 p-0 backdrop-blur-sm md:p-6" onClick={closeLayer}>
    <div data-safe-mode-work="true" className="ui-modal-enter flex h-[100dvh] w-full max-w-7xl flex-col overflow-hidden bg-white shadow-2xl dark:bg-gray-950 md:h-[92vh] md:rounded-2xl md:border md:border-gray-700 lg:flex-row" onClick={event => event.stopPropagation()}>
      ...
      <section className="flex min-h-0 w-full flex-col border-l border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 lg:w-[560px]">
        <header className="flex flex-none items-center gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-800"><div className="min-w-0 flex-1"><p className="text-[11px] font-bold uppercase tracking-wider text-indigo-500">灵感档案</p><h2 data-safe-mode-title="true" className="truncate text-lg font-bold text-gray-950 dark:text-white">{draft.title}</h2></div><RatingStars value={draft.rating || 0} onChange={editable ? value => update('rating', value) : undefined} compact /><IconButton label="关闭" onClick={closeLayer} className="hidden lg:inline-flex"><X /></IconButton></header>
