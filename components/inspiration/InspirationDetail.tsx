...
import { Archive, Check, ChevronDown, Copy, Download, ExternalLink, ImagePlus, MoreHorizontal, Palette, Pin, Play, Sparkles, UserRound, Wand2, X } from 'lucide-react';
            ...
            <div className="mb-1 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <ImageTaggerAction
                  ...
                  onInsert={(newTags) => {
                    const combined = normalizeInspirationTags([...splitTags(tagText), ...splitTags(newTags)]);
                    const nextText = combined.join(', ');
                    setTagText(nextText);
                    update('tags', combined);
                    notify(`已追加 ${splitTags(newTags).length} 个反推 Tag`);
                  }}
                />
                {editable && <button type="button" onClick={() => { const tags = suggestInspirationTags({ ...draft, tags: splitTags(tagText) }); update('tags', tags); setTagText(tags.join(', ')); }} className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600"><Wand2 className="h-3.5 w-3.5" />自动整理</button>}
              </div>
            </div>
            <input disabled={!editable} value={tagText} onChange={event => setTagText(event.target.value)} onBlur={() => update('tags', splitTags(tagText))} placeholder="构图, 光影, 角色, 待尝试" className="h-11 w-full rounded-xl border border-gray-200 bg-white px-3 text-sm dark:border-gray-800 dark:bg-gray-900" />
            {splitTags(tagText).length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{splitTags(tagText).map(tag => <span key={tag} className="rounded-full bg-indigo-50 px-2.5 py-1 text-meta font-semibold text-indigo-700 dark:bg-indigo-950/50 dark:text-indigo-300">#{tag}</span>)}</div>}
          </div>
