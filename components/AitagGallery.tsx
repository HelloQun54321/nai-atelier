        ...
        <div className="hidden min-w-0 flex-1 items-center gap-2 md:flex">
          ...
          <div className="relative flex-none">
            ...
            {showDesktopFilters && <div role="dialog" aria-label="AITag 筛选" className="absolute left-1/2 top-[calc(100%+0.5rem)] z-50 hidden w-[min(34rem,calc(100vw-2rem))] -translate-x-1/2 rounded-xl border border-gray-200 bg-white p-4 shadow-2xl dark:border-gray-700 dark:bg-gray-800 md:block">
              ...
              <label className="mb-3 block text-xs text-gray-500 dark:text-gray-400">Prompt 搜索<input value={prompt} onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { handleSearch(); setShowDesktopFilters(false); } }} placeholder="搜索 NovelAI 元数据 Prompt" className="mt-1.5 w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800 outline-none dark:border-gray-600 dark:bg-gray-900 dark:text-white" /></label>
