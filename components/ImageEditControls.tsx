...
export const ImageEditControls: React.FC<ImageEditControlsProps> = ({
  operation, draft, layout = DEFAULT_LAB_PAGE_LAYOUTS[operation], fileInputRef, selectableParams, strength, noise, brushSize, focused, minimumContextArea, tool, expansion, canvasSize = { width: 0, height: 0 }, isBusy = false, apiKey, notify,
      ...
      <LabModuleSection moduleId="editSettings" label="编辑参数" order={getModuleOrder(layout, 'editSettings')} defaultCollapsed={isModuleCollapsed(layout, 'editSettings')}>
        <section className="space-y-4 rounded-lg border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/45">
          ...
          {operation === 'outpaint' && <>
            ...
            <button type="button" role="switch" aria-checked={manualMaskEditing} disabled={isBusy} onClick={() => onManualMaskEditingChange(!manualMaskEditing)} className="flex w-full items-center justify-between gap-3 rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-left text-xs font-semibold text-gray-600 transition hover:border-indigo-300 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-indigo-700"><span><span className="block">手动调整蒙版</span><span className="mt-0.5 block text-[10px] font-normal text-gray-400">默认只生成新增边缘；开启后可用画笔修正接缝区域</span></span><span className={`relative h-5 w-9 flex-none rounded-full transition-colors ${manualMaskEditing ? 'bg-indigo-500' : 'bg-gray-300 dark:bg-gray-700'}`}><span className={`absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${manualMaskEditing ? 'translate-x-4' : 'translate-x-0'}`} /></span></button>
          </>}
          {(operation === 'inpaint' || operation === 'outpaint' && manualMaskEditing) && <>
            {operation === 'inpaint' && <label className="flex items-center justify-between gap-3 text-xs font-semibold text-gray-600 dark:text-gray-300"><span>Focused Inpainting</span><input disabled={isBusy} type="checkbox" checked={focused} onChange={event => onFocusedChange(event.target.checked)} className="h-4 w-4 accent-amber-500" /></label>}
            <div className="flex items-center gap-2"><button disabled={isBusy} type="button" onClick={() => onToolChange('brush')} className={`flex h-9 flex-1 items-center justify-center gap-1 rounded-lg text-xs ${tool === 'brush' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300' : 'bg-gray-100 text-gray-500 dark:bg-gray-900'}`}>画笔</button><button disabled={isBusy} type="button" onClick={() => onToolChange('eraser')} className={`flex h-9 flex-1 items-center justify-center gap-1 rounded-lg text-xs ${tool === 'eraser' ? 'bg-gray-200 text-gray-800 dark:bg-gray-800 dark:text-gray-200' : 'bg-gray-100 text-gray-500 dark:bg-gray-900'}`}><Eraser className="h-3.5 w-3.5" />橡皮擦</button></div>
            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300">{focused ? '框选区域后在内部绘制蒙版' : '笔刷大小'} {!focused && <span className="float-right font-mono">{brushSize}px</span>}{!focused && <input disabled={isBusy} type="range" min="8" max="512" step="4" value={brushSize} onChange={event => onBrushSizeChange(Number(event.target.value))} className="mt-2 w-full accent-indigo-500" />}</label>
            {focused && <div className="space-y-2"><label className="block text-xs font-semibold text-gray-600 dark:text-gray-300">最小上下文 <span className="float-right font-mono">{minimumContextArea}px</span><input disabled={isBusy} type="range" min="32" max="96" step="8" value={minimumContextArea} onChange={event => onMinimumContextAreaChange(Number(event.target.value))} className="mt-2 w-full accent-amber-500" /></label><button disabled={isBusy} type="button" onClick={onResetFocusedRect} className="h-8 w-full rounded-lg bg-amber-50 text-xs font-semibold text-amber-700 hover:bg-amber-100 dark:bg-amber-950/30 dark:text-amber-300">重新框选区域</button></div>}
            <div className="grid grid-cols-4 gap-1"><button disabled={isBusy} type="button" onClick={onUndo} className="flex h-9 items-center justify-center rounded-lg bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300 disabled:opacity-50" title="撤销"><RotateCcw className="h-4 w-4" /></button><button disabled={isBusy} type="button" onClick={onRedo} className="flex h-9 items-center justify-center rounded-lg bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300 disabled:opacity-50" title="重做"><RotateCw className="h-4 w-4" /></button><button disabled={isBusy} type="button" onClick={onClearMask} className="flex h-9 items-center justify-center rounded-lg bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300 disabled:opacity-50" title="清空蒙版"><Trash2 className="h-4 w-4" /></button><button disabled={isBusy} type="button" onClick={onInvertMask} className="flex h-9 items-center justify-center rounded-lg bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300 disabled:opacity-50" title="反转蒙版"><Contrast className="h-4 w-4" /></button></div>
          </>}
        </section>
      </LabModuleSection>
