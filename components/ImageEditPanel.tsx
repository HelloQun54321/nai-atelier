...
export const ImageEditPanel: React.FC<ImageEditPanelProps> = ({
  ...
  return (
    <div className="chain-editor-body flex min-h-0 flex-1 flex-col overflow-y-auto bg-white dark:bg-gray-900 lg:flex-row-reverse lg:overflow-hidden">
      <div className="relative order-1 flex min-h-[42vh] flex-1 flex-col items-center justify-center overflow-auto bg-gray-100 p-3 dark:bg-black/20 md:p-6 lg:order-2">
        ...
        {!state.width && <div className="absolute inset-x-6 bottom-6 rounded-lg border border-dashed border-gray-300 bg-white/90 px-3 py-3 text-center text-xs text-gray-500 dark:border-gray-700 dark:bg-gray-900/90 dark:text-gray-400">请先生成图片、选择历史图片或导入 PNG、JPEG、WebP 底图</div>}
        {error && <div role="alert" className="mt-3 w-full max-w-xl rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-center text-xs text-red-600 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-300">{error}</div>}
        <div className="mt-4 flex w-full flex-none flex-col items-center">
          <div className="mb-2 text-xs text-gray-500 dark:text-gray-400">{operation === 'image-to-image' ? '图生图' : operation === 'inpaint' ? '局部重绘' : '扩图'} · 结果会保存为新的历史图片</div>
          <button type="button" onClick={() => void submit()} disabled={isLoading || !baseImage} className="generation-action-button flex h-11 w-full max-w-xs items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold disabled:opacity-50">生成编辑结果 <span className="generation-action-button__cost">{generationCostLabel(operation, focused)}</span></button>
        </div>
      </div>
      <div className="order-2 w-full shrink-0 overflow-y-auto border-t border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 lg:order-1 lg:w-1/2 lg:flex-1 lg:border-r lg:border-t-0">
        ...
        <div className="space-y-4">
          <label className="block text-sm font-semibold text-gray-800 dark:text-gray-100">本次 Prompt<textarea value={draft.prompt} onChange={event => onPromptChange(event.target.value)} className="mt-2 min-h-28 w-full resize-y rounded-lg border border-gray-300 bg-gray-50 p-3 font-mono text-sm leading-relaxed text-gray-900 outline-none focus:ring-1 focus:ring-indigo-500/50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" placeholder="输入本次实际要生成的完整提示词" /></label>
        ...
        <div className="mt-5 rounded-lg border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/45">
        ...
        <div className="mt-5 space-y-4">
          ...
          {operation === 'outpaint' && <div className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900"><div className="mb-2 text-xs font-semibold text-gray-700 dark:text-gray-200">扩展画布（像素）</div><div className="grid grid-cols-2 gap-2">{(['top', 'right', 'bottom', 'left'] as const).map(side => <label key={side} className="text-[11px] text-gray-500 dark:text-gray-400">{({ top: '上', right: '右', bottom: '下', left: '左' } as const)[side]}<input type="number" min="0" step="64" value={expansion[side]} onChange={event => setExpansion(previous => { const next = { ...previous, [side]: Math.max(0, Number(event.target.value) || 0) }; onDraftChange({ expansion: next }); return next; })} className="mt-1 h-9 w-full rounded-lg border border-gray-300 bg-white px-2 text-sm dark:border-gray-700 dark:bg-gray-900" /></label>)}</div><button type="button" onClick={() => void applyOutpaint()} className="mt-3 h-9 w-full rounded-lg bg-indigo-600 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-500">应用画布扩展</button></div>}
        </div>
      </div>
    </div>
  );
