...
export const CharacterReferenceManager: React.FC<Props> = ({ params, setParams, markChange, notify, operation }) => {
  ...
  return <>
    ...
    {open && <div className="workspace-manager fixed inset-0 z-[1100] flex flex-col bg-gray-50 text-gray-900 dark:bg-gray-950 dark:text-gray-100">
      ...
      {detail ? <main className="min-h-0 flex-1 overflow-y-auto p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-gray-900 sm:px-5">
          <input value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索角色参考…" className="mobile-touch min-w-40 flex-1 rounded-xl border border-gray-200 bg-transparent px-3 text-sm dark:border-gray-800" />
          <button type="button" onClick={() => setArchived(value => !value)} className={`mobile-touch rounded-xl px-3 text-xs font-bold ${archived ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900' : 'border border-gray-200 dark:border-gray-800'}`}>{archived ? '查看资料库' : '已归档'}</button>
