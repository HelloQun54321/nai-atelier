...
export const CharacterReferenceManager: React.FC<Props> = ({ params, setParams, markChange, notify }) => {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[10000] flex flex-col bg-gray-50 text-gray-900 dark:bg-gray-950 dark:text-gray-100">
      ...
      {detail ? <main className="min-h-0 flex-1 overflow-y-auto p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-6">
        <div className="mx-auto grid max-w-5xl gap-5 md:grid-cols-[minmax(280px,1fr)_minmax(300px,1fr)]">
        ...
        <main className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="overflow-y-auto p-3 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-5">
            {loading ? <div className="py-20 text-center text-sm text-gray-500">加载中…</div> : assets.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">{assets.map(asset => { const selected = references.slots.some(slot => slot.assetId === asset.id); return <article key={asset.id} className={`overflow-hidden rounded-2xl border bg-white shadow-sm dark:bg-gray-900 ${selected ? 'border-cyan-500 ring-2 ring-cyan-500/20' : 'border-gray-200 dark:border-gray-800'}`}>
          ...
          <aside className="overflow-y-auto border-t border-gray-200 bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] dark:border-gray-800 dark:bg-gray-900 lg:border-l lg:border-t-0">
