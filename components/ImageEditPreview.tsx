...
export const ImageEditPreview: React.FC<ImageEditPreviewProps> = ({ operation, baseImage, error, generationCostLabel, onGenerate, ...canvasProps }) => (
    ...
    <div className="chain-editor-preview order-1 flex min-h-0 w-full shrink-0 flex-1 flex-col border-b border-gray-200 bg-gray-100 dark:border-gray-800 dark:bg-black/20 lg:order-2 lg:w-1/2 lg:border-b-0">
      <div className="flex min-h-[400px] flex-1 flex-col overflow-hidden p-4 md:p-6">
        ...
        <div className="mt-4 flex flex-none flex-col items-center">
          ...
          <button type="button" onClick={onGenerate} disabled={canvasProps.isLoading || !baseImage} className="generation-action-button flex h-11 w-full max-w-xs items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold disabled:opacity-50">生成编辑结果 <span className="generation-action-button__cost">{generationCostLabel}</span></button>
        </div>
      </div>
    </div>
  </div>
