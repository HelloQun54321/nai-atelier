...
export const Layout: React.FC<LayoutProps> = ({ children, onNavigate, currentView, activeView = currentView, isDark, themeMode, setThemeMode, appearancePreferences, setAppearancePreferences, safeMode, safeModeStartup, safeModeHideTitles, setSafeModeStartup, setSafeModeHideTitles, toggleSafeMode, toast, hideNav, notify, onOpenAgent }) => {
  ...
  const compactAnlasBudget = anlasBudget.loading
    ? '…'
    : anlasBudget.remaining < 10_000
      ? String(anlasBudget.remaining)
      : new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 1 }).format(anlasBudget.remaining).toLowerCase();
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('nai-open-global-settings', openSettings);
  }, []);
      ...
      <aside style={{ width: sidebarCollapsed ? SIDEBAR_COLLAPSED_WIDTH : sidebarWidth }} className={`app-sidebar relative hidden flex-shrink-0 flex-col border-r border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950 md:flex ${isSidebarResizing ? '' : 'transition-[width] duration-200'}`}>
        ...
        <div className={`flex flex-none flex-col gap-2 border-t border-gray-200 py-2 dark:border-gray-800 ${sidebarCollapsed ? 'px-2' : 'px-3'}`}>
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-gray-50/70 dark:border-gray-800 dark:bg-gray-900/70">
            <div role="status" title={sidebarCollapsed ? `Anlas 预算：${anlasBudget.loading ? '加载中' : anlasBudget.remaining}` : undefined} aria-label={`Anlas 预算 ${anlasBudget.loading ? '加载中' : anlasBudget.remaining}`} className={`flex h-11 w-full cursor-default select-none items-center border-b border-gray-200 text-indigo-600 dark:border-gray-800 dark:text-indigo-300 ${sidebarCollapsed ? 'justify-center px-0' : 'justify-between px-3'}`}>
              {sidebarCollapsed
