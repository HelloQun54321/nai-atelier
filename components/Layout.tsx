...
interface LayoutProps {
  children: ReactNode;
  onNavigate: (view: 'list' | 'characters' | 'edit' | 'library' | 'aitag' | 'inspiration' | 'admin' | 'history' | 'playground', id?: string) => void;
  currentView: string;
  activeView?: string;
  isDark: boolean;
  toggleTheme: () => void;
...
export const Layout: React.FC<LayoutProps> = ({ children, onNavigate, currentView, activeView = currentView, isDark, toggleTheme, currentUser, onLogout, toast, hideNav }) => {
  // 使用统一的角色策略获取存储配额
  const getMaxStorage = () => {
