<div align="center">
  ...
  **面向 NovelAI 的个人本地创作工坊**
  ...
  [![Version](https://img.shields.io/badge/version-1.0.8-6366f1?style=flat-square)](./CHANGELOG.md)
  ...
  [项目定位](#-项目定位) · [首次使用](#-首次使用) · [界面预览](#-界面预览) · [功能地图](#-功能地图) · [创作工作流](#-核心创作工作流) · [资源资料库](#-tag-与资源资料库) · [st-chatu8 互通](#-sillytavern--st-chatu8-互通nai-atelier-连接器) · [手机访问](#-手机局域网访问) · [数据存储](#-本地数据与图片存储) · [日常启动](#-日常启动) · [数据备份](#-备份与数据持久化) · [更新日志](./CHANGELOG.md)
...
工坊以「生成一张 → 观察 → 微调 → 再生成」为默认节奏，帮助创作者保留判断与选择。常规探索优先使用免费面积与步数范围，复用已有 Vibe；需要消耗 Anlas 时先展示估算与确认。V5 免费档仍会消耗 Opus 限额，本地预算也不等于官方余额。
...
>
> 💡 **术语说明**：文档与工坊界面中的 **Anlas** 即 NovelAI 官方代币（通俗常称“积分”或“点数”）；**Opus 限额** 即官方为 V5 模型引入的电池型免费生成配额。
...
| 📚 **资料库可自生长** | 个人资料由使用过程持续沉淀，基础 Tag 目录按需从上游下载更新 |
...
- 准备有效的 NovelAI **Opus** 订阅及 `pst-` 开头的 API Key；电脑需要能访问 NovelAI。Agent、Pixiv 与公共队列均为可选配置。
...
启动脚本按需构建前端与 Worker，并启动本地网关和词库更新服务。等待终端显示服务就绪后，打开 [http://localhost:3000](http://localhost:3000)；使用期间保持启动终端运行。首次使用不需要部署到 Cloudflare，也不需要配置云端 D1/R2。
...
5. 在「系统设置 → 数据与维护」配置备份位置；手机访问与桌面快捷方式见后文。
...
> [!NOTE]
> - **代表性页面展示**：以下截图展示了工坊核心功能页面的**完整界面视图**；因篇幅有限，仅精选部分代表性模块进行展示，更多功能（全景画师库、角色库、生图历史瀑布流、灵感库、全局设置等）可直接在本地运行体验。
...
模型能力按官方应用运行时同步，内置清单用于离线回退；未知模型采用保守能力判断。当前内置能力的主要区别如下，实际可用选项以界面同步结果为准：

| 能力 | V4 | V4.5 | V5 |
| --- | --- | --- | --- |
| Vibe Transfer | 支持 | 支持 | 不支持 |
| Precise Reference | 不支持 | 支持 | 不支持 |
| 多角色提示词上限 | 6 | 6 | 32 |
...
- **尺寸与费用估算**：提供竖图、横图、方图预设和 64 像素步进调节。推荐竖图 `832 × 1216` 为 1,011,712 像素；默认免费面积上限为 1,048,576 像素，恰好对应 `1024 × 1024`。实际门槛从官方应用同步，是否扣费还取决于模型、步数、参考资产与当前 Opus 状态。
- **步数与采样锁定**：默认将 Steps 上限严格锁定在免费门槛内（**V5 系列作者实测推荐并默认 23 步，V4/V4.5 系列跟随官方锁定 28 步**），减少因调高步数跨入 Anlas 计费范围的风险；免费步数并不保证整次请求免费。
- **透明通道生成**：在支持该能力的模型（当前内置为 V5）下开启透明背景生成，适配立绘、二创贴纸与表情包制作。
- **PNG / JSON 参数导入**：拖入保留 NovelAI 元数据的 PNG 原图或兼容 JSON，提取可识别的 Prompt、Seed、采样器与 CFG 等参数并回填；被平台移除元数据的图片无法还原完整配置。
...
- **Focused Inpainting（聚焦重绘）**：围绕蒙版区域保留上下文、裁切放大，生成后羽化回贴，适合面部和小范围细节修补；结果仍需检查边缘衔接。费用按实际请求与模型能力估算，以生成前的显示为准。裁切与合成实现见 [imageEdit.ts](./services/imageEdit.ts)。
- **空蒙版防跑拦截**：画板未涂抹任何笔迹（全透明蒙版）时，前端直接拦截生成并友好提示，避免空蒙版请求。
...
- **独立草稿与蒙版状态隔离**：文生图、图生图、局部重绘与扩图四种模式的底图、蒙版、提示词与调参草稿各自独立保存；切换时恢复对应模式草稿，防止跨模式复用错误蒙版；需长期保存的配置仍应收入资料库。
- **移动端三段式 Tab 流程**：窄屏视口下自动收敛为高效的三段式 Tab 导航（图生图为「底图/提示/参数」，局部重绘为「画板/提示/参数」，扩图为「画布/提示/参数」）；画笔工具箱贴身附着于画板边缘，减少长距离滑动与画板涂抹的手势冲突。

### 📌 预设引用：按板块记录来源

在编辑器右上角点击「引用」，可以按需单独借用其他风格串的局部资产：

- **细粒度按需引用**：可单独勾选引用画风、全局提示词、模块、角色、负面词或生成参数，各板块清晰显示其原始来源。
...
- **复用已有原图**：从历史或已缓存 AITag 收录时直接复用电脑中的原图路径，避免重复拷贝。
...
- **本地高强度加密**：LLM API Key 采用 AES-256-GCM 加密保存在电脑本地（密钥文件 `local-data/prompt-agent.key`），局域网重置密码不影响解密；调用时由电脑向所配置的模型供应商发送鉴权信息，不向手机返回已保存的明文凭据。
- **安全请求头注入**：自定义兼容接口支持安全附加请求头（如 `HTTP-Referer`），敏感鉴权头严格走加密存储，避免明文混入普通配置。
...
- **精准调参避雷**：精细修改全局提示词、负面词、提示词模块、多角色坐标、采样器与 Seed，未提及的高级参数安全保留。
- **全局环境控制**：支持切换界面主题、安全模式遮罩、图片布局与手机缓存，任务完成后可自动导航至目标页面。

#### 安全沙盒与事务提交

- **纯业务沙盒隔离**：Agent 仅拥有工坊提供的结构化业务工具，不提供任意文件读写、命令行或系统进程工具；联网能力仅限本轮搜索提取的公网 HTTPS 页面，严格阻断内网与私有端口。
...
实验室角色管理模块内置 NovelAI 官方所称的 **Precise Reference**（项目当前内置能力表中由 **V4.5 系列**支持，V4 与 V5 不支持；V5 的多角色提示词与自由坐标定位是另一项能力）。参考图原件保存在电脑，手机列表只读取缩略图；生图时由电脑读取原图、适配官方画布并组装请求，手机无需保存原图或单独开启 VPN。
...
全局设置的「NovelAI 与 Anlas」区域可以启用与 st-chatu8 兼容的公共队列。使用相同 NovelAI Key、并接入同一公共服务的客户端会进入同一条生成队列；轮到当前任务后，电脑才通过自己的网络和 VPN 请求 NovelAI。
...
- 遵循“看过留下、点开保存”的自生长缓存哲学，告别机械式全量挂机索引
- 按需轻量拉取并分别记录作品摘要、详情、首图和完整图片缓存状态
- 卡片用不同色调表示未缓存、已缓存首图和已缓存全部
- 按收藏、首图缓存或完整缓存过滤
- aitag.win 暂时不可用时自动切换到本地缓存
- 已缓存的作品、详情和图片在离线状态下仍可完整浏览与复用
...
- **送往实验室**：任何页面反推完成后，选中 Tag 可一键写入实验室全局提示词并自动跳转。
...
| 未授权直接请求静态图片或 API | 由网关校验并拒绝未授权请求 |
...
- 底部常驻核心主入口：**风格串**、**资源库**（集成画师库、角色库、AITag、Danbooru、Pixiv 与灵感库）、**生图实验室**、**生成历史**与**系统设置**。
...
| **画师库** | 双列预览网格，一键抽卡灵感直达，多维标签与流派筛选收入底部抽屉 |
...
本项目保存的作品与核心资料以电脑持久层为准；手机负责显示与操作。st-chatu8 历史原图是例外：工坊保存索引，文件仍在 SillyTavern 原目录。
...
- **按需 WebP 转码**：列表按卡片渲染宽度在 160～960px 六档中动态请求合适的 WebP 缩略图，自动校正 EXIF 旋转方向，减少列表加载量。
- **原图与缩略图分开保存**：缩略图用于列表加速，原图用于查看、下载与参数导入。元数据与透明度取决于原始文件；外部图片不一定是 PNG 或含有 NovelAI 参数，st-chatu8 桥接原图仍由其原目录提供。
- **智能淘汰与封面常驻**：电脑端缩略图缓存池默认上限 1 GB，超出后按 LRU（最近最少使用）自动释放；**固定封面不参与普通缓存的 LRU 淘汰**，但仍应备份其原始资产。
- **列表丝滑预热**：在浏览 Pixiv 或 Danbooru 时，网关在收到列表的第一时间就会以低并发在后台把下一批预览图预写到磁盘，减少滚动翻页时的等待；打开画师目录时亦会自动预取候选封面并固定落盘。
...
- **三重遮罩防护**：开启后所有画作与作品封面默认应用高强度模糊、深度压暗与饱和度归零，降低旁人直接看到图片的风险；主动打开大图查看时不遮挡作品。
- **临时窥视机制**：点击单张图片仅临时解除遮罩预览，不会误触发弹窗详情；鼠标移开对应区域后恢复模糊。
- **状态失焦自锁**：切换标签页、应用失去焦点、页面最小化或轻按 `Esc`，遮罩立即全自动重置闭合。
- **配置持久可控**：支持在系统设置中设定「启动时默认开启防窥」，手机与桌面端均可一键即时启闭。

### 多密钥保管箱与额度监控（Key Vault & Quota Tracker）
...
#### 🔋 Anlas 积分预算与 Opus 电池限额监控机制

工坊桌面端侧边栏左下角常驻呈现 **Anlas 点数预算** 与 **Opus 电池限额环**（移动端收录于设置及顶栏状态提示），针对创作者最关心的资产与限额消耗提供全透明、免打扰的实时守护：

- **Opus 电池限额：全自动轮询获取（用户无需手动填写）**：
  - **官方接口原生支持**：NovelAI 官方开放了 `/user/subscription` 订阅端点，原生支持使用 API Key 查询当前账号的订阅有效性与 Opus 限额。
...
- **Anlas（官方代币/积分）：用户自主设定本地预算**：
  ...
  - **本地自动扣减与超额预警**：生图（如超出免费面积、启用角色参考或 Opus 限额不足等）或 Vibe 编码成功后，工坊会在本地自动扣减对应点数。当预算耗尽（0 点）或剩余不足以支付本次生成时，系统会弹出确认拦截面板警示，防止不知情下过度扣除云端点数（本地扣减仅用于超额预警与消耗追踪，不影响云端账户）。
- **每个 Key 分别计算与记录**：
  - **按密钥指纹分别记录**：系统以 Key 的 SHA-256 指纹区分本地预算、Opus 快照与用量统计。切换 Key 时显示对应记录，排队也使用对应指纹；这些是同一工坊中的逻辑分组，不是独立数据库或多用户安全沙箱。
  - **拼车公共队列独立排队**：多人拼车公共队列亦按各 Key 的 SHA-256 指纹独立排队节流，各车队各自调度，互不影响。
- **透支报警与防误扣保护**：
  - **三色状态感知**：额度充裕时圆环呈翡翠绿，低于 20% 时呈琥珀黄预警，用尽透支（≤0%）时标红示警。
  - **扣费透明阻断**：额度耗尽后生图将正式扣除 Anlas 积分。此时生图按钮会从「消耗额度 / 免费」明确切换为「消耗 XX 点」，并在生图前弹出确认面板列明详细扣费清单，帮助创作者在提交前判断费用；共享账号的额度可能继续变化，最终扣费以官方结算为准。
  - **失效 Key 硬拦截**：若账号订阅到期（官方返回 `active: false`），圆环立即变红并显示 `×`，直观提示该 Key 已失效并引导切换，不将订阅失效显示为满额。
...
**使用 st-chatu8 桥接时，还需单独备份 SillyTavern 中被索引的原图及相关数据。** 工坊只记录外部文件路径，`local-data` 备份不包含这些原图；迁移后需恢复原目录关系，或重新配置桥接并核对图片可读。

### 应用内备份

1. 打开「系统设置 → 数据与维护 → 重要数据备份」。
2. 设置电脑上的备份目标绝对路径，建议选择另一块磁盘；不能放在 `local-data` 内部。手机填写的也是电脑路径。
3. 等待生成、保存和同步任务结束，暂时停止其他设备操作，填写可选备注后点击「立即完整备份」。
4. 等待完成，在「历史备份存档」中定位并核对文件。每份存档包含 `local-data/` 和 `backup-metadata.json`。

当前内置备份逐文件复制，**不提供数据库事务快照，也不会暂停所有后台写入**。需要迁移或留存一致的恢复副本时，使用下面的停服复制方式。
...
3. 将备份中的完整 `local-data/` 复制到新项目根目录。目标已有同名数据时先停止操作，另选空目录，避免覆盖或混合两套数据库。
4. 安装依赖后运行 `npm run dev:local`，检查风格串、历史原图、Key 配置与 Agent 会话；使用桥接时也检查外部原图。词库缺失可重新下载。
5. 验证通过后再决定是否切换日常启动入口。在验证前保留原项目与备份，不以界面能打开作为恢复完整的唯一依据。
