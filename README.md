<div align="center">
  ...
  **面向 NovelAI 的个人本地提示词、角色、画师与图片工作台**
  ...
  [项目定位](#-项目定位) · [相对原版的改造](#-相对原项目的主要改造) · [功能地图](#-功能地图) · [手机局域网](#-手机局域网访问) · [手机 UI](#-手机-ui-设计) · [数据备份](#-备份与恢复) · [更新日志](./CHANGELOG.md)
</div>
...
- Agent 只有 NaiPromptManager 提供的结构化业务工具，没有电脑文件、命令行、系统进程、浏览器或任意网址权限；所有项目读取继续受四位局域网认证保护。
...
- 支持在输入框直接附加最多 4 张 PNG/JPEG/WebP/GIF 图片；图片只随当前请求发送给支持识图的模型，不写入会话持久记录，电脑服务负责处理，手机不保存原图。
...
- 附件发送前后都会检查当前模型是否支持识图；不支持识图的模型会禁用上传入口并明确提示。Agent 结束时还会检查实验室版本，避免覆盖用户在后台期间手动修改的内容。
