...
| 🗄️ 多密钥保管箱 | 多把 NovelAI Key 本地命名保管、随时热切换、失效/格式感知，密钥本地 D1 加密收纳 |
| 🔋 积分预算与限额监控 | Opus 电池限额通过 Key 全自动轮询与动态张数换算；Anlas 积分预算由用户自主设定（官方 Key 无查账权限），各 Key 独立记账核算与透支防护 |
...
实验室内置永久 Vibe 资料库（**专属于 NovelAI V4 / V4.5 系列模型**，V5 系列官方暂未开放 Vibe 支持）。它会调用 NovelAI V4.5 Full 的 Vibe 编码接口，将参考图转换成不可逆编码；每个新的 Information Extracted 数值编码一次需消耗 **2 Anlas**，以后使用该编码生图不再重复支付编码费用。
...
实验室角色管理模块内置 NovelAI 官方所称的 **Precise Reference**（**专属于 NovelAI V4 / V4.5 系列模型**，官方 V5 暂未支持精确参考，但 V5 原生支持多达 32 个角色的自由坐标定位）。参考图原件保存在电脑，手机列表只读取缩略图；生图时由电脑读取原图、适配官方画布并组装请求，手机无需保存原图或单独开启 VPN。
...
- **Opus 电池限额：全自动轮询获取（用户无需手动填写）**：
  ...
  - **后台休眠与智能唤醒**：当切换其他标签页或浏览器最小化（`document.hidden`）时，轮询自动挂起以节约系统资源；切回工坊瞬间全自动补发一次静默刷新。发起生图前 15 秒强制核验最新快照，点击圆环亦可随时即时刷新。
- **Anlas（官方代币/积分）：用户自主设定本地预算（受官方 API 权限限制）**：
  - **为什么需要用户自己填写？**：由于 NovelAI 官方的安全权限划分，创作者持有的持久 API Key（`pst-...`）仅具备生图与基础订阅查询权限，**官方并没有向 API Key 开放查询云端账户实际 Anlas 余额的接口**（查看真实云端积分必须在官网进行完整的账号密码登录）。因此，任何纯基于 API Key 的工具都无法自动读取你的云端 Anlas 余额。
  - **本地安全预算守卫机制**：创作者在「系统设置 → Anlas 点数预算」中填入自己的实际点数或期望分配给工坊的安全预算（默认 1666 点，可随时修改或一键恢复）。电脑与手机局域网共享该预算。
  - **本地自动扣减与超额预警**：生图（如非免费尺寸、高步数、角色参考等）或 Vibe 编码成功后，工坊会在本地自动扣减对应点数。当预算耗尽（0 点）或剩余不足以支付本次生成时，系统会弹出确认拦截面板警示，防止不知情下过度扣除云端点数（本地扣减仅用于超额预警与消耗追踪，不影响云端账户）。
- **每个 Key 独立计算与沙箱隔离**：
  - **按密钥指纹物理隔离**：系统为每把 Key 计算唯一的 SHA-256 哈希身份指纹。在保管箱切换 Key 时，无论是该账号自主设定的 Anlas 本地预算与扣减水位、云端自动获取的 Opus 电池限额环与剩余张数，还是个人历史用量统计（本机已花 Anlas、已搓 Opus 免费张数），全部**毫秒级切入目标账号独立沙箱**，不同账号之间数据完全物理隔离、互不串扰。
  - **拼车公共队列独立排队**：多人拼车公共队列亦按各 Key 的 SHA-256 指纹独立排队节流，各车队各自调度，互不影响。
- **透支报警与防误扣保护**：
  - **三色状态感知**：额度充裕时圆环呈翡翠绿，低于 20% 时呈琥珀黄预警，用尽透支（≤0%）时瞬间标红示警。
  - **扣费透明阻断**：额度耗尽后生图将正式扣除 Anlas 积分。此时生图按钮会从「消耗额度 / 免费」明确切换为「消耗 XX 点」，并在生图前弹出确认面板列明详细扣费清单，彻底杜绝创作者在毫不知情的情况下被意外扣除 Anlas 积分。
  - **失效 Key 硬拦截**：若账号订阅到期（官方返回 `active: false`），圆环立即变红并显示 `×`，直观提示该 Key 已失效并引导切换，绝不呈现虚假的满额状态误导生图。
