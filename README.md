...
- 🎨 **画师tag** - 接入每日更新的画师目录，支持中英文搜索、权重组合与本地按需生成预览。
...
| 🔎 **Tag 自动补全** | 实验室提示词输入支持 31 万余条中英对照 Danbooru Tag 与 NovelAI V4.5 专属 Tag 联想；可用中英文搜索，选择后插入英文，并保留权重语法。 |
...
- **画师tag**：
  - 从本地中英 Tag 快照读取 146,835 个画师 Tag，首次展示 500 位热门画师，向下滚动时每次自动追加 500 位；也可用中英文直接搜索完整目录。
  - 卡片显示 Danbooru 关联作品数作为热度，并支持热度升降序和名称 A–Z / Z–A 四种完整目录排序。
  - 不再从第三方静态图片仓库导入；缺少预览时可用 NovelAI V4.5 按统一测试提示词生成并保存到本地。
  - 支持 `artist:` 前缀开关，支持 `{}` `[]` 权重语法。
  - **随机抽卡**：从完整画师目录按 6 / 12 / 24 位抽取，支持惊喜混合、完全随机和热门画师三种卡池，最近五批会自动避重，并可随时返回原目录位置。
  - **批量选择**：支持从文本把多个画师 Tag 加入当前组合。
- **灵感图库 (Inspirations)**：
  - 云端存储高质量生成的图片。
...
### 前置要求
- Node.js 18+（仅重新生成 Tag 词库需要 Node.js 22+）
- Cloudflare 账号
...
电脑和手机连接同一家庭网络时，启动窗口会显示类似 `http://192.168.0.105:3000` 的手机访问地址及四位数字密码。手机浏览器打开该地址并输入密码即可使用电脑中的同一份数据。

- 电脑本机通过 `localhost` 访问时免密。
- 手机等局域网设备首次访问需要四位密码，成功后在该设备保持授权30天。
- 连续输错5次会暂停验证1分钟；数据接口和本地图片都受密码保护。
- 四位密码与会话密钥只保存在 `local-data/lan-access.json`，不会提交到 Git。
- 移动端使用独立的紧凑导航和双列资源布局；建议把访问地址加入手机浏览器书签。
- 仅用于可信家庭网络，请勿配置路由器端口转发或暴露到公网。
...
更新过程会收录该数据库中使用量不少于 10 的 Tag，并把 NovelAI V4.5 官方专属 Tag 合并到本地分片词库。候选支持英文和中文前缀搜索，同时显示英文原名与中文解释；选择候选时只插入 NovelAI 可识别的英文 Tag。更新词库需要 Node.js 22+。
...
- 双击启动器后，电脑上的图片网关监听 `3000` 端口，内部 Worker 仅监听本机 `3001` 端口；电脑和手机的访问地址不变。
- 手机列表页按需读取 320px 或 640px WebP 缩略图，打开详情、下载和读取元数据时仍使用未经转码的原图。
- 电脑缩略图保存在 `local-cache/thumbnails`，上限 1 GB；该目录是可重新生成的缓存，不属于重要数据，也不需要备份。
- 手机默认最多保留 100 MB 小图缓存，可在“全局设置 → 手机图片缓存”中改为关闭、25 MB、50 MB 或 100 MB，并可随时清空。
- 手机缓存只包含可重新获取的小图；原图、历史记录、画师串和灵感数据仍以电脑的 `local-data` 为准。
- 手机生图请求由电脑网关转发给 NovelAI，会使用电脑当前的系统网络/VPN；手机本身不需要再连接 VPN。若电脑使用的只是浏览器代理扩展而不是系统代理或 VPN，电脑服务进程可能无法使用该代理。
