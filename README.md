...
- 会话会记录任务运行、完成、失败、中止和重启后中断状态；上下文接近模型限制时按 Token 预算裁剪旧消息，项目名称、Prompt、Tag 和 AITag 文本按不可信资料处理，不会被当作 Agent 指令。
- 电脑会保留最近 200 条任务事件；手机或浏览器短暂断线后重新打开会话，可以看到任务是否中断及可恢复提示，而不是只剩一个没有过程的状态标签。
...
- 预设与 Vibe 工具查询电脑上的完整资料库，不受发送给模型的前200条摘要限制；项目概况由数据库轻量统计，不读取封面和 Base64 原图。
