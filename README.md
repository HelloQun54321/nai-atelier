...
- 图片可以再次导入实验室、发布到灵感或下载原图。
- 历史数据保存在电脑，不依赖浏览器缓存。

### 灵感库：经过筛选的个人参考集

- 保存标题、图片、Prompt、负面 Prompt 和生成参数。
- 支持从历史发布，也支持从 AITag 直接收藏。
- 上传 NovelAI PNG 时自动读取可识别的元数据。
- 可以重新导入实验室继续修改和生成。
- 与历史不同：历史负责完整记录，灵感负责主动筛选与长期复用。
