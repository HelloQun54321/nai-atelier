<div align="center">
  ...
  **面向 NovelAI 的个人本地创作工坊**
  ...
  [![Version](https://img.shields.io/badge/version-0.79.41-6366f1?style=flat-square)](./CHANGELOG.md)
...
| 🌌 **Vibe Transfer** | 永久保存 NovelAI 画风编码并组合强度 | 实验室、画师串、历史 |
| 🧬 **角色参考** | 保存人物参考图并使用 NovelAI Precise Reference | 实验室、画师串、历史 |
...
画师串是项目的核心数据结构，用于把一套可复用的创作配置保存下来。
...
画师串并不是单纯的“画师名字列表”，它更接近一份可以不断修改、组合和引用的 NovelAI 创作预设。
...
- 可以搜索本地中英 Tag、画师串、角色串、画师资料、灵感、AITag、永久 Vibe、角色参考与组合，并创建或更新相应的项目资料。
...
- Agent 现在使用独立的全局多会话工作台：可以新建、切换、搜索、自动命名、手动重命名和删除对话；会话不再绑定某个画师串，电脑与手机可以接着同一条对话继续。
...
- Agent 每次需要预设时先搜索轻量摘要，再按 ID 读取单条画师串或角色串的完整模块、参数、Vibe 与角色参考快照；局部修改会与原参数合并，不会因为只改步数而清空尺寸、采样器或高级引用。Vibe 由工具按 ID 查询电脑资料库，浏览器不再在每条消息里重复上传整批预设和 Vibe。
- 项目工具还支持角色 Tag 专用搜索、角色参考重命名与恢复、从历史创建待编码 Vibe、从历史设置画师串封面，以及把电脑已缓存的 AITag 图片导入灵感、角色参考、Vibe 或画师串封面；来源始终限制在项目内部图片接口，不开放任意网址或电脑文件。
...
- “删除”采用归档：资料库暂时隐藏资产，但画师串和历史中的旧快照仍能继续复现。
- 画师串、实验室预设和生成历史保存独立选择快照，之后修改全局组合不会反向改变旧配置。
...
- 画师串、灵感和历史随完整参数保存参考 ID、名称、类型和数值快照；归档只从默认资料库隐藏，不会删除旧记录。
...
| 数据 | 方向 | 规则 |
| --- | --- | --- |
| 画师串 Prompt | 双向 | 同名或已关联资料冲突时以 st-chatu8 为准 |
| 画师串配图 | 双向 | 保留原封面；只有图片真正变化时才重新上传 |
...
NAI Atelier 独有的新画师串可以先同步到 st-chatu8；一旦进入 st-chatu8，后续修改和冲突判断都以 st-chatu8 数据为准。这样在 SillyTavern 聊天生图和本项目实验室之间切换时，不需要手工复制画师串或 Vibe。
...
- 同一张灵感图可继续创建画师串、角色参考、Vibe，或设为现有画师串封面。
...
| **保存到画师串** | 把图片、Prompt、负面 Prompt 和参数保存为可编辑预设 |
...
```mermaid
flowchart TD
    A["画师串 / 角色 / 灵感 / 历史记录"] --> D["local-data 中的 D1"]
