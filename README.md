...
- 画师 Tag 和角色 Tag 卡片可左右切换查看同一精确 Tag 的热门候选图（按 Danbooru 热度降序）；角色候选会继续排除多人图、漫画、文字梗、玩偶和特殊服装等干扰。首次默认图仍优先体现角色典型外观；选中某张候选后可点图钉设为固定封面，图片会保存到项目自己的存储中。老师、提督等没有可靠单人形象的泛角色宁可留空，不强行误配；候选结果缓存 14 天。
