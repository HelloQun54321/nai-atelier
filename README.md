...
| 🔋 积分与限额自动监控 | 自动轮询 Anlas（官方积分）余额与 Opus 电池限额，无需手动填写；按各 Key 独立核算、动态换算剩余张数并提供透支防误扣保护 |
...
#### 🔋 Anlas（官方积分）与 Opus 电池限额全自动监控

工坊桌面端侧边栏左下角常驻呈现 **Anlas 积分余额** 与 **Opus 电池限额环**（移动端收录于设置及顶栏状态提示），针对创作者最关心的资产与限额消耗提供全透明、免打扰的实时守护：

- **全自动获取，用户完全无需手动填写**：
  - **用户只需填 Key**：创作者在保管箱中录入自己的 NovelAI API Key 后，**完全不需要手动填写、核算或更新任何积分余额与限额数据**。
  - **后台静默轮询**：工坊后台每 60 秒自动向 NovelAI 官方订阅端点同步一次数据，全自动解析获取当前账号的 Anlas 真实余额、Opus 会员有效状态以及电池限额百分比。
  - **后台休眠与智能唤醒**：当切换到其他标签页或浏览器最小化（`document.hidden`）时，轮询自动挂起以节约系统资源与网络请求；切回工坊页面瞬间全自动补发一次静默刷新。
  - **生图前强制快照核验**：发起生图前，若本地快照超过 15 秒，系统会自动强制拉取一次官方最新额度，防止多人拼车或多端并发时因状态滞后导致误扣费。
  - **即时手动刷新**：点击侧栏圆环或余额区域，可随时立即强制刷新当前账号的最新数据。
- **每个 Key 独立计算与沙箱隔离**：
  - **按密钥指纹物理隔离**：系统为每把 Key 计算唯一的 SHA-256 哈希身份指纹。在保管箱切换 Key 时，侧栏 Anlas 余额、Opus 限额环进度、预估剩余张数、本地安全预算以及个人历史统计（已消耗点数、已搓免费张数）全部**毫秒级切入目标账号独立沙箱**，不同账号之间数据完全物理隔离、互不串扰。
  - **拼车公共队列独立排队**：多人拼车公共队列亦按各 Key 的 SHA-256 指纹独立排队节流，各车队各自调度，互不影响。
  - **各 Key 专属本地预算守卫**：每个 Key 可单独设置本地 Anlas 安全预算阈值（默认 1666 点），当账号积分消耗触达自设红线时强行阻断生图，防止拼车或实验时把账号点数意外耗尽。
- **Opus 电池限额机制与剩余张数动态换算**：
  - **什么是 Opus 电池限额**：自 NovelAI V5 系列起，官方为 Opus 会员引入了动态免费限额池（V4.5 及以下经典模型仍享无限量免费）。额度通常以百分比呈现（满额 100%），随时间持续自动恢复；在官方活动加成期间还可能突破上限（如 196%），工坊完整支持真实数值呈现而不作粗暴截断。
  - **动态换算剩余可搓张数**：工坊结合从官方应用热同步提取的换算系数（默认约 17.3 张/1%），在限额环下方直观显示预估可生成的免费张数（如 `剩余 28% ≈ 484 张`），无需创作者心算百分比。
- **透支报警与防误扣保护**：
