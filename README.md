...
实验室是实际调参生图的主控台：将预设、外部素材与当前灵感组装为生成请求，直连 NovelAI 实时获得高分辨率渲染反馈。

#### 全局与角色内容
- **全局图层**：基础画风、功能提示词模块、主体场景描述与全局负面提示词。
- **角色图层**：支持多角色同框管理，各角色拥有独立正负面词，提供 AI 自动构图与画板手动拖拽坐标定位；角色预设可直接从资料库一键导入。
- **角色参考（Precise Reference）**：支持直选「角色」、「画风」或「角色与画风」模式。

#### 尺寸控制与参数调优
- **免费像素动态锁**：预设竖屏、横屏、方形与 64 像素步进微调；拖动单边可按比例联动锁定，在榨干 Opus 免费像素上限的同时，实时显示总像素与计费预估，绝不多花一点冤枉点数。
- **图生图 / 重绘 / 扩图底图适配**：支持从历史记录调取、本地上传或拖入底图；底图尺寸不合规时自动居中补齐并对齐官方步长。
- **修图提示词隔离**：图生图与局部重绘采用独立的整图提示词输入区，绝不会误将文生图残留的多角色提示词与坐标带入重绘接口。
- **图片参数一键反解**：拖入任意 NovelAI 官方原图或 JSON 元数据，瞬间反解出完整 Prompt 与模型参数；支持按画风/主体智能拆分导入，或单一全局提示词原样导入。
