 ...
 */
import { computeNaiRuntimeSync, DEFAULT_NAI_RUNTIME } from './media-gateway.mjs';

const SOURCE = 'https://novelai.net/image';

const fetchText = async (url, timeoutMs = 30_000) => {
...
try {
  ...
  const chunks = await Promise.all(paths.map(path => fetchText(`https://novelai.net${path}`).catch(() => '')));
